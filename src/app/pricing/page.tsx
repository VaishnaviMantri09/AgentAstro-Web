    ...
    const plans = [
        {
            ...
            popular: false,
            features: [
                { icon: HiDocumentReport, text: 'Basic Report Depth.', highlight: false },
                { icon: BiSupport, text: 'Limited SLA Support.', highlight: true },
                { icon: GrCompliance, text: 'Core Regulatory Compliance.', highlight: false },
            ]
        },
        {
            name: 'Team Plan',
            ...
            features: [
                { icon: HiDocumentReport, text: 'Advanced Report Depth.', highlight: true },
                { icon: BiSupport, text: 'Increased SLA Support.', highlight: true },
                { icon: GrCompliance, text: 'Advanced Regulatory Compliance.', highlight: false },
            ]
        },
        {
            ...
            features: [
                { icon: HiDocumentReport, text: 'Comprehensive Report Depth.', highlight: true },
                { icon: BiSupport, text: '24/7 Priority Support.', highlight: true },
                { icon: RiUser3Fill, text: 'Dedicated Onboarding Specialist.', highlight: true },
                { icon: GrCompliance, text: 'Enterprise Regulatory Compliance.', highlight: false },
            ]
        },
    ];
        ...
        >
            ...
            <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16 relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
                            {[
                                ...
                                <div key={index} className={`flex flex-col items-center p-6 sm:p-8 bg-white/80 backdrop-blur-lg rounded-xl shadow-lg border border-blue-100 hover:bg-white/90 transition-all duration-500 transform hover:-translate-y-2 sm:hover:-translate-y-4 hover:scale-105 hover:shadow-2xl group animate-scale-in ${feature.delay} w-full`}>
                                    ...
                                    <h3 className="font-gesta text-2xl sm:text-3xl font-semibold text-gray-800 mb-3 sm:mb-4 group-hover:text-blue-600 transition-colors duration-300 text-center">{feature.title}</h3>
                                    <p className="font-gesta text-lg sm:text-xl text-gray-600 text-center transition-colors duration-300 group-hover:text-gray-700 leading-relaxed">{feature.description}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className={`flex flex-col items-center justify-center mb-10 sm:mb-16 transition-all duration-1000 ${isVisible ? 'animate-fade-in-up stagger-4' : 'opacity-0'}`}>
                    <h2 className="font-gesta text-3xl sm:text-4xl font-bold text-gray-100 mb-8 sm:mb-12 text-center animate-bounce-gentle">Choose Your Subscription</h2>
                    <div className="flex items-center gap-3 sm:gap-4 animate-scale-in stagger-1">
                        <span className={`font-gesta text-lg sm:text-xl font-medium transition-all duration-300 ${!isYearly ? 'text-black transform scale-110' : 'text-white'}`}>
                            Monthly
                        </span>
                        <button
                            ...
                            >
                                <div className="p-6 sm:p-8 flex flex-col h-full">
                                    <div className="text-center mb-4 sm:mb-6">
                                        <h3 className="font-gesta text-2xl sm:text-3xl font-bold text-gray-800 mb-2 group-hover:text-blue-600 transition-all duration-300 transform group-hover:scale-110">{plan.name}</h3>
                                        <p className="font-gesta text-lg sm:text-xl text-gray-600 font-medium group-hover:text-blue-500 transition-colors duration-300">{plan.users}</p>
                                    </div>

                                    <div className="text-center mb-6 sm:mb-8">
                                        <div className="mb-2">
                                            <span className="font-gesta text-4xl sm:text-5xl font-bold text-gray-800 group-hover:text-blue-600 transition-all duration-500 transform group-hover:scale-110 inline-block">
                                                ${isYearly ? plan.yearlyPrice : plan.monthlyPrice}
                                            </span>
                                            <span className="font-gesta text-gray-600 text-xl sm:text-2xl ml-2 group-hover:text-blue-500 transition-colors duration-300">
                                                /mo
                                            </span>
                                        </div>
                                        {isYearly && (
                                            <p className="font-gesta text-gray-500 text-xl sm:text-2xl group-hover:text-blue-400 transition-colors duration-300 animate-fade-in">
                                            ...
                                            {plan.features.map((feature, featureIndex) => {
                                                ...
                                                return (
                                                    <div key={featureIndex} className="flex items-center gap-3 group-feature">
                                                        ...
                                                        <span className={`font-gesta text-lg sm:text-xl transition-all duration-300 group-feature-hover:translate-x-1 ${feature.highlight
                                            ...
                                            <span className="relative z-10 transition-all duration-300 flex items-center justify-center gap-2">
                                                {loading === plan.name ? (
                                                    <>
                                                        ...
                                                        <span className="font-gesta text-lg sm:text-xl">Processing...</span>
                                                    </>
                                                ) : (
                                                    'Subscribe Now'
                                                )}
