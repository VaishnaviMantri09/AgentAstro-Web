...
const ArticlePage = () => {
  ...
  const similarArticles = articles.filter((a) => a.slug !== slug).slice(0, 11);
    ...
    >
      ...
      <header className="relative max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-10 md:py-16 lg:py-24 flex flex-col items-start lg:flex-row lg:items-center gap-10 lg:gap-24">
        <div className="lg:w-1/2 flex justify-center lg:justify-start w-full">
          {article.image && (
            <div className="relative w-full max-w-[900px] h-48 sm:h-72 md:h-96 rounded-3xl shadow-2xl ring-offset-transparent hover:ring-offset-indigo-700 transition">
              <Image
                src={article.image}
        ...
        <div className="lg:w-1/2 mt-6 md:mt-10 lg:mt-0 max-w-xl space-y-4 md:space-y-6 pl-4 md:pl-8 ">
          ...
          <p className="font-gesta text-xl text-gray-100 font-semibold leading-relaxed drop-shadow">
            {article.excerpt}
          </p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 mb-6 md:mb-5 flex flex-col lg:flex-row gap-8 lg:gap-12">
        <section className="w-full lg:w-2/3">
          <article
            className="bg-gray-800 bg-opacity-60 backdrop-blur-md rounded-3xl shadow-xl p-4 md:p-10 xl:p-14 animate-fadeIn"
        ...
        <aside className="w-full lg:w-1/3 flex-shrink-0">
          <div className="bg-gray-900 bg-opacity-70 rounded-3xl shadow-lg p-4 md:p-6 border">
            <h3 className="font-gesta text-xl font-bold text-gray-100 mb-4 md:mb-5">Similar Articles</h3>
            <ul className="space-y-4 md:space-y-6">
              {similarArticles.map((sim) => (
                <li key={sim.slug} className="border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
                  <button
                    onClick={() => router.push(`/updates/${sim.slug}`)}
                    className="block text-left w-full"
                  >
                    <p className="text-gray-100 font-semibold font-gesta">{sim.title}</p>
                    <p className="text-[#EBEEF2] text-sm mt-1 line-clamp-2 font-gesta">{sim.excerpt}</p>
                  </button>
                </li>
              ))}
        ...
        <div className="mx-auto max-w-4xl text-center">
          <div className="relative">
            ...
            <div className="relative  rounded-xl p-12 border border-white/30 ">
              ...
              <p className="font-gesta text-xl text-black leading-relaxed mb-6">
      ...
      <style jsx global>{`
        ...
        .prose h2::before {
          ...
          width: 6px;
          height: 4rem;
          background: linear-gradient(135deg, #EBEEF2, #EBEEF2);
          border-radius: 4px;
          font-family: 'Gesta', sans-serif;
        }
        .prose h3 {
