    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800">
        <div className="text-center px-6">
          <h1 className="text-5xl font-extrabold text-red-500 mb-6">404 - Not Found</h1>
    ...
    >
      ...
      <header className="relative max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-10 md:py-16 lg:py-24 flex flex-col items-start lg:flex-row lg:items-center gap-10 lg:gap-24">
        ...
        <div className="lg:w-1/2 mt-6 md:mt-10 lg:mt-0 max-w-xl space-y-4 md:space-y-6 pl-4 md:pl-8 ">
          <div className="flex flex-wrap items-center gap-2 md:gap-4 text-sm text-white/90 mb-4 md:mb-6">
            <span className="bg-black backdrop-blur-sm px-3 py-1 rounded-full font-medium">
              {article.category}
            </span>
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold leading-tight tracking-wide text-indigo-50 drop-shadow-md ">
            {article.title}
          </h1>
          <p className="text-lg md:text-xl text-indigo-900 font-semibold leading-relaxed drop-shadow">
            {article.excerpt}
          </p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 mb-6 md:mb-5 flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <aside className="w-full lg:w-1/3 flex-shrink-0">
          <div className="bg-gray-900 bg-opacity-70 rounded-3xl shadow-lg p-4 md:p-6 border border-indigo-800">
            <h3 className="text-lg md:text-xl font-bold text-indigo-200 mb-4 md:mb-5">Similar Articles</h3>
            <ul className="space-y-4 md:space-y-6">
              {similarArticles.map((sim) => (
                <li key={sim.slug} className="border-b border-indigo-700 pb-4 last:border-b-0 last:pb-0">
                  ...
                  >
                    <p className="text-indigo-100 font-semibold">{sim.title}</p>
                    <span className="text-sm text-indigo-300">{sim.date}</span>
                    <p className="text-indigo-400 text-xs mt-1 line-clamp-2">{sim.excerpt}</p>
                  </button>
                </li>
              ))}
              {similarArticles.length === 0 && (
                <li className="text-indigo-300">No similar articles found.</li>
              )}
            </ul>
          </div>
        </aside>
      ...
      <section className="px-3 py-10 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <div className="relative">
            ...
            <div className="relative bg-gradient-to-r from-white/15 to-white/10 backdrop-blur-sm rounded-3xl p-12 border border-white/30 shadow-2xl">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href='/started'>
                  <button className="group bg-white text-blue-600 px-8 py-4 rounded-full font-semibold hover:bg-blue-50 transition-all duration-300 flex items-center justify-center gap-2 hover:scale-105 shadow-xl">
                    Join Today
      ...
      <style jsx global>{`
        ...
        @media (max-width: 640px) {
          .prose p, .prose ul, .prose ol, .prose li { font-size: 1rem; }
