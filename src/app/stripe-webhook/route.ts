...
export async function POST(request: NextRequest) {
    ...
    try {
        const rawBody = await request.text(); // webhook requires raw text
    ...
    if (event.type === "checkout.session.completed") {
        ...
        if (customerEmail) {
            try {
                // Create token here if you want (JWT or random string in DB)
                const token = encodeURIComponent(Buffer.from(`${customerEmail}:${Date.now()}`).toString("base64"));

                // Example link for account creation page
...
async function sendAccountEmail(to: string, link: string) {
    // Transporter (use your SMTP provider)
