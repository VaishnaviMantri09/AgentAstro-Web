...
const StoryPage = () => {
  ...
  const faqData = [
    ...
    },
    {
      ...
      answer: 'We offer three subscription tiers: Starter Tier (ideal for smaller companies or consultants, with limited queries per month), Professional Tier (for growing teams needing regular access and deeper reports), and Enterprise Tier (for high-volume users with full access, white-glove support, and optional per-query add-ons). Pricing and tier details are available on Pricing Page also available via a meeting.',
    },
    {
  ...
  const timelineItems = [
    ...
    },
    {
      ...
      content: 'Our founders, frustrated with traditional research methods during their own FDA submissions, envisioned AI that could understand regulatory patterns and improve decision making.',
            ...
            <div className="max-w-4xl mx-auto">
              <div
                className={`relative overflow-hidden rounded-3xl p-8 backdrop-blur-sm border transition-all duration-700 transform ${visibleItems.has(activeItem) ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  } ${timelineItems[activeItem].gradient} ${timelineItems[activeItem].border} bg-blue-900 border-blue-900 hover:scale-105 hover:shadow-2xl group`}
              >
                <div className="relative z-10">
                  <h3
                    className={`text-3xl font-bold mb-6 ${timelineItems[activeItem].textColor} transition-all duration-500`}>
                    {timelineItems[activeItem].title}
                  </h3>
                  <p className="text-blue-100/90 text-lg leading-relaxed">{timelineItems[activeItem].content}</p>
                </div>
              </div>
            </div>
          </div>
                      ...
                      <div className="absolute top-20 left-1/2 transform -translate-x-1/2 w-32 text-center">
                        <div
                          className={`px-3 py-2 rounded-lg backdrop-blur-sm border transition-all duration-300 ${activeItem === index
                            ? `bg-blue-900 ${item.gradient} ${item.border} scale-105`
                            : 'bg-white/5 border-white/20 hover:bg-white/10'
                            }`}
                        >
                          <p className={`text-sm font-bold ${activeItem === index ? item.textColor : 'text-black'}`}>
                            {item.title}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="ml-6 flex-1 min-w-0">
                          <div
                            className={`px-4 py-3 rounded-xl backdrop-blur-sm border transition-all duration-300 ${activeItem === index
                              ? `bg-blue-900 ${item.gradient} ${item.border} scale-105 shadow-xl`
                              : 'bg-white/5 border-white/20 hover:bg-white/10'
                              }`}
                          >
                            <h3 className={`text-base font-bold leading-tight ${activeItem === index ? item.textColor : 'text-white'
                              }`}>
                              {item.title}
                            </h3>
                          </div>
            ...
            {faqData.map((faq, index) => (
              ...
              >
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 hover:bg-white/15 transition-all duration-500 overflow-hidden hover:scale-105 hover:shadow-xl hover-glow animate-slideInLeft card-3d">
                  ...
                  >
                    <span className="text-lg font-semibold text-white pr-4 group-hover:text-blue-200 transition-colors duration-300">{faq.question}</span>
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ${openFAQ === index ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="px-8 pb-6">
                      <div className="text-blue-100 leading-relaxed animate-fadeIn animate-slideInUp">{faq.answer}</div>
                    </div>
                  </div>
