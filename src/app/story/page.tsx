    ...
    <div className="space-y-2">
      {lines.map((line, index) => {
        if (/^\d+\.\s/.test(line.trim())) {
          const formattedLine = line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-black font-bold">$1</strong>');
          return (
            <div key={index} className="ml-4 flex">
              <span className="text-black font-bold mr-2">{line.match(/^\d+\./)?.[0]}</span>
              <span className="text-black" dangerouslySetInnerHTML={{ __html: formattedLine.replace(/^\d+\.\s/, '') }} />
            </div>
          );
        }
          ...
          return (
            <div key={index} className="ml-4 flex">
              ...
              <span className="text-black" dangerouslySetInnerHTML={{ __html: formattedLine.replace(/^•\s/, '') }} />
            </div>
          );
        }
        else if (line.trim()) {
          const formattedLine = line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-black font-bold">$1</strong>');
          return (
            <p key={index} className="text-black" dangerouslySetInnerHTML={{ __html: formattedLine }} />
          );
        }
        return null;
      })}
            ...
            <div className="max-w-4xl mx-auto">
              <div
                className={`relative overflow-hidden rounded-3xl p-8 backdrop-blur-sm border transition-all duration-700 transform ${visibleItems.has(activeItem) ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  } ${timelineItems[activeItem].gradient} ${timelineItems[activeItem]} bg-[#EBEEF2] border-blue-900 hover:scale-105 hover:shadow-2xl group`}
              >
                      ...
                      <div className="absolute top-20 left-1/2 transform -translate-x-1/2 w-32 text-center">
                        <div
                          className={`px-3 py-2 rounded-lg backdrop-blur-sm border transition-all duration-300 ${activeItem === index
                            ? `bg-black ${item.gradient} scale-105`
                            : 'bg-white/5 border-white/20 hover:bg-white/10'
                            }`}
                        >
                          <p className={`font-gesta text-base font-bold ${activeItem === index ? 'text-white' : 'text-black'}`}>
                            {item.title}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="ml-6 flex-1 min-w-0">
                          <div
                            className={`px-4 py-3 rounded-xl backdrop-blur-sm border transition-all duration-300 ${activeItem === index
                              ? `bg-black ${item.gradient} scale-105 shadow-xl`
                              : 'bg-white/5 border-white/20 hover:bg-white/10'
                              }`}
                          >
                            <h3 className={`font-gesta text-base font-bold leading-tight ${activeItem === index ? 'text-white' : 'text-black'
                              }`}>
                              {item.title}
                            </h3>
                          </div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-4xl w-full mx-auto mb-20">
            {teamMembers.map((member, index) => (
              ...
              >
                <div className="bg-[#EBEEF2] rounded-3xl p-8 transition-all duration-500 hover:scale-110 hover:rotate-1 text-center relative overflow-hidden animate-float-slow h-full flex flex-col">
                  <div
                    className={`absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-100 transition-opacity duration-500 animate-gradient`}
                  ></div>
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center flex-grow">
                    ...
                    <p className="text-lg font-gesta text-[#061A40] font-semibold mb-4 animate-fadeIn text-center" style={{ animationDelay: `${index * 0.1 + 0.2}s` }}>
                      {member.role}
                    </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {advisors.map((advisor, index) => (
              ...
              >
                <div className="bg-[#EBEEF2] rounded-3xl p-6  transition-all duration-500 text-center  h-full flex flex-col group-hover:scale-105 transition-all duration-500">
        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="space-y-4">
            {faqData.map((faq, index) => (
              ...
              >
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 hover:bg-white/15 transition-all duration-500 overflow-hidden animate-slideInLeft card-3d">
                  <button
                  ...
                  >
                    <span className="font-gesta text-xl font-bold text-gray-900 pr-4 group-hover:text-gray-700 transition-colors duration-300">{faq.question}</span>
                    <div className="flex-shrink-0">
                      {openFAQ === index ? (
                        <ChevronUp className="w-5 h-5 text-gray-700 transition-all duration-300 rotate-180 animate-bounce-slow" />
                      ) : (
                        <ChevronDown className="w-5 h-5 text-gray-700 transition-all duration-300 group-hover:animate-wiggle" />
                      )}
                    </div>
                  </button>
                  <div className={`overflow-hidden transition-all duration-500 ${openFAQ === index ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="px-8 pb-6">
                      <div className="font-gesta text-lg text-black leading-relaxed animate-fadeIn animate-slideInUp">
                        <FormattedAnswer content={faq.answer} />
                      </div>
                    </div>
                  </div>
                </div>
